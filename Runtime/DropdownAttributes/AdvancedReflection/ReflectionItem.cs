...
namespace YusamPackage.DropdownAttributes.AdvancedReflection
{
    public class ReflectionItem
    {
        ...
        public ReflectionItem(object baseMaster, object master, string target)
        {
            ...
            debug(
                $"<b><color=teal>[Reflection Progress]</color></b> Master class: <b>{MasterType.Name}</b><color=olive>(INSTANCE)</color> {ReflectionSystem.arrowSymbol} Target: <b>{Target}</b><color=olive>({ReflectionItemType})</color> \n");
        }
        ...
        public ReflectionItem(object baseMaster, Type masterType, string target)
        {
            ...
            ReflectionItemType = DetectReflectionItemType(target); //Detect the type first
            debug(
                $"<b><color=teal>[Reflection Progress]</color></b> Master class: <b>{MasterType.Name}</b><color=olive>(STATIC CLASS)</color> {ReflectionSystem.arrowSymbol} Target: <b>{Target}</b><color=olive>({ReflectionItemType})</color> \n");
        }
        ...
        private ReflectionItemType DetectReflectionItemType(string Target)
        {
            ...
            if (couldBeInstance && couldBeClass)
            {
                //WARNING
                Debug.LogError(
                    $"Reflection confused. the target {Target} in {MasterType} could be instance or class at the same time!");
            ...
            try
            {
                switch (ReflectionItemType)
                {
                    ...
                    case ReflectionItemType.CLASS:
                        Debug.LogError(
                            "Please use GetStaticClass() instead of GetValue() if the ReflectionItemType is Class");
        ...
        private object FindMethodResult()
        {
            ...
            debug(
                $"<color=darkblue><b>[Invoke method]</b></color> <b>{MasterType.Name}.{Target}</b>  Result: <b>{result.ToStringWithQuotes()}</b>\n");
        ...
        public object[] GetParametersFromTarget()
        {
            string
                parameterPart =
                    Target.Split(new[] { '(' }, 2)
                        [1]; //"GetName(minecraft, getPlayer(world, uuid))"   ->   "minecraft, getPlayer(world, uuid))"
            parameterPart =
                parameterPart.Substring(0,
                    parameterPart.Length -
                    1); //"minecraft, getPlayer(world, uuid))"   ->   "minecraft, getPlayer(world, uuid)"
